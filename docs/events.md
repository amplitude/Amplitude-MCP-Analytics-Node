...
| `rate_limited` | An otherwise-unclassified thrown error carried HTTP status 429 (timeout and transport classifications take precedence). Also sets `retrySuggested` on `ctx.error` |
