...
core of low-level utilities (module resolution, privacy redaction, serverless
flush accounting, delivery contract) that is too small to justify a separate
shared package and too risky to reinvent.
...
| `src/core/privacy.ts` | `src/core/privacy.ts` | `97ea346abd0caf333a3bafbd26b74de1d545f3e7` | Redaction primitives (PII patterns, base64-image redaction, content hash, recursive `sanitizeStructuredContent`) verbatim. Dropped the agent/LLM-domain surface (`$llm_message` chunking, `sanitize{SystemPrompt,ReasoningContent,ToolDefinitions}`, `normalizeToolDefinitions`, `contentMode` tiers) — those key off upstream `./constants.js` PROP_* event constants and are out of scope, same boundary `debug.ts` draws. `PrivacyConfig` keeps the `privacyMode`/`redactPii`/custom-redaction machinery but exposes taxonomy-free value transforms (`redactText`/`redactValue`) instead of methods returning event-property dicts. Wired into the emit seam (`ctx-to-properties`) via `MCPAnalyticsConfig.toPrivacyConfig()`: free-form content (caller properties + `ctx.extra`) is redacted; typed identity/dimension fields are not. |
